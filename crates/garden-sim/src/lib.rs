...
use garden_hal::{HalError, SensorBank};
...
impl SensorBank for Simulation {
    fn capabilities(&self) -> garden_core::CapabilitySet {
        self.state.sensors.capabilities()
    }

    fn read(&mut self) -> Result<garden_core::SensorSnapshot, HalError> {
...
mod tests {
    use super::*;
    use garden_core::{DueWindow, RuleId, Severity};
