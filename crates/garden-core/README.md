...
parameter rather than being read, which is what lets `garden-sim` run a 120-day season
...
```rust
use garden_core::{Capability, CapabilitySet};
...
```rust
use garden_core::{Target, TaskKey, TaskKind};
...
```rust
use garden_core::{VarietyBook, VarietyId};
