        ...
        Some(sensors) => {
            ...
            if let Some(rate) = store
                .fitted_consumption_lpd(
                    garden.id,
                    &state.tank_geometry,
                    garden_core::time::add_days(now, -CONSUMPTION_WINDOW_DAYS),
                    now,
                )
                .await?
            {
...
async fn overlay_vision(
    ...
    let cutoff = garden_core::time::add_days(now, -VISION_STALE_DAYS);
...
mod tests {
    ...
    use garden_core::{SlotId, VarietyId};
    ...
    async fn a_sensorless_garden_still_produces_calendar_advice() {
        ...
        let planted = garden_core::time::add_days(t0(), -90.0);
        store
            .plant(
                garden.id,
                SlotId(0),
        ...
        store
            .record_planting_event(
                ...
                garden_core::time::add_days(t0(), -76.0),
            )
            .await
            .unwrap();

        let state = build(&store, &garden, t0()).await.unwrap();
        let evaluation = garden_rules::default_engine().evaluate(&state);
        ...
        assert!(evaluation.has(garden_core::TaskKind::Harvest));
    }
    ...
    async fn measure(
        ...
        let frame = store
            .put_frame(garden_store::frames::NewFrame {
                garden: garden.id,
                captured_at: at,
                width: 1,
                height: 1,
                ...
                comparable: true,
                source: garden_store::frames::FrameSource::Agent,
                bytes: PNG,
            })
            .await
            .unwrap()
        ...
        let mut metrics = garden_core::SlotMetrics::new(slot, at, area);
    ...
    async fn a_recorded_tank_action_reaches_the_rule_engine() {
        ...
        store
            .plant(
                ...
                SlotId(0),
                &VarietyId::new("basil"),
                garden_core::time::add_days(t0(), -10.0),
                16,
                None,
            )
            .await
        ...
        store
            .record_tank_event(garden.id, garden_core::TankEvent::DeepClean, None, t0())
            .await
            .unwrap();
    ...
    async fn feeding_the_tank_silences_the_feeding_task() {
        ...
        store
            .plant(
                ...
                SlotId(0),
                &VarietyId::new("basil"),
                garden_core::time::add_days(t0(), -30.0),
                16,
                None,
            )
            .await
        ...
        store
            .record_planting_event(
                ...
                garden_core::PlantingId(1),
                garden_store::plantings::PlantingEvent::Germinated,
                garden_core::time::add_days(t0(), -20.0),
            )
            .await
            .unwrap();
        ...
        assert!(
            before.has(garden_core::TaskKind::AddPlantFood),
        ...
        store
            .record_tank_event(
                garden.id,
                garden_core::TankEvent::FedToStrength { strength: 1.0 },
                None,
                t0(),
            )
            .await
        ...
        assert!(
            !after.has(garden_core::TaskKind::AddPlantFood),
    ...
    async fn a_camera_that_stopped_reporting_drops_the_capability_by_itself() {
        ...
        let later = garden_core::time::add_days(t0(), 5.0);
    ...
    async fn one_dark_night_does_not_flap_the_capability_off() {
        ...
        let tomorrow = garden_core::time::add_days(t0(), 1.0);
    ...
    async fn measured_harvest_supersedes_the_calendar_once_the_camera_is_calibrated() {
        ...
        let (store, garden) = fixture(DeviceModel::Studio2).await;
        let planted = garden_core::time::add_days(t0(), -20.0);
        store
            .plant(
                garden.id,
                SlotId(0),
        ...
        store
            .record_planting_event(
                ...
                garden_core::PlantingId(1),
                garden_store::plantings::PlantingEvent::Germinated,
                garden_core::time::add_days(t0(), -14.0),
            )
            .await
            .unwrap();

        let before = garden_rules::default_engine()
        ...
        let after = garden_rules::default_engine()
            .evaluate(&build(&store, &garden, t0()).await.unwrap());
    ...
    async fn a_simulated_garden_gets_telemetry_overlaid() {
        ...
        let state = build(&store, &garden, garden_core::time::add_days(t0(), 40.0))
            .await
            .unwrap();
        assert!(has_telemetry(&state));
