...
use garden_core::{Capability, DeviceModel, Garden, GardenState, SlotId, Timestamp, VarietyId};
use garden_sim::Simulation;
use garden_sim::scenario::Operator;
...
/// [`Stage::Seeded`](garden_core::Stage::Seeded) — the gate on thinning, harvest,
...
pub async fn seed_plantings(
    ...
    let book = garden_core::VarietyBook::starter();
    ...
    for planting in store.active_plantings(garden.id).await? {
        ...
        let sprouted = garden_core::time::add_days(
            planting.planted_at,
...
fn run(garden: &Garden, now: Timestamp) -> Option<GardenState> {
    ...
    let elapsed = garden_core::time::days_between(garden.created_at, now);
    ...
    garden_sim::scenario::run(&mut sim, Operator::TYPICAL, days, seed);
...
pub async fn ensure_frame(
    ...
    let due = match store.latest_frame(garden.id).await? {
        None => true,
        Some(latest) => {
            garden_core::time::days_between(latest.captured_at, now)
    ...
    let stored = store
        .put_frame(garden_store::frames::NewFrame {
            ...
            // on real hardware.
            comparable: true,
            source: garden_store::frames::FrameSource::Simulated,
            bytes: &bytes,
        })
        .await?;
...
mod tests {
    ...
    fn a_real_garden_gets_no_invented_telemetry() {
        ...
        state.capabilities = garden_core::CapabilitySet::empty();
    ...
    fn canopy_is_only_reported_for_slots_that_hold_a_plant() {
        ...
        let g = garden(DeviceModel::Simulated);
        let mut state = GardenState::for_garden(g.id, t0());
        state.plantings.push(garden_core::Planting::new(
