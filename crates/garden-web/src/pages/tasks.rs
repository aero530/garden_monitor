...
use garden_auth::{Actor, Permission, TaskAction};
use garden_core::{GardenId, TaskKey};
...
fn parse_task_kind(label: &str) -> Option<garden_core::TaskKind> {
...
fn planting_target(target: &str) -> Option<garden_core::PlantingId> {
    target
        .strip_prefix("planting ")?
        .trim()
        .parse()
        .ok()
        .map(garden_core::PlantingId)
}

/// Map the stored task kind label back to a planting event.
fn kind_to_event(kind: &str) -> Option<garden_store::plantings::PlantingEvent> {
    use garden_core::TaskKind::*;
    let parsed = [PruneRoots, PrunePlant, Harvest, Thin]
        .into_iter()
        .find(|k| k.label() == kind)?;
    garden_store::plantings::event_for_task(parsed)
}
...
async fn one_tap(
    ...
    let Some(token) = garden_auth::SecretToken::from_client(&raw) else {
