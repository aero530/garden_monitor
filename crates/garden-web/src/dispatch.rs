...
use garden_auth::{Permission, TaskAction};
use garden_core::{Garden, Severity, TaskKey, TaskKind};
use garden_notify::{NotificationAction, compose, compose_brief, decide, reach_for};
...
async fn sweep(state: &AppState) -> Result<(), garden_store::StoreError> {
    let now = state.now();
...
async fn deliver_brief(
    ...
    if let Some(last) = state.store.last_notified(garden.id, user.id, &key).await?
        && garden_core::time::days_between(last.at, now) < 0.9
    {
        return Ok(());
    }
...
async fn build_actions(
    ...
    key: &TaskKey,
    now: Timestamp,
) -> Result<Vec<NotificationAction>, garden_store::StoreError> {
