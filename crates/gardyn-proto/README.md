...
```sh
cargo test -p gardyn-proto    # 12 tests
```
...
```mermaid
sequenceDiagram
  participant P as gardyn-edge<br/>(Raspberry Pi)
  participant B as gardyn-web<br/>(the brain)

  Note over P,B: every request carries<br/>Authorization: Bearer + x-gardyn-protocol
...
```rust
use gardyn_proto::{TelemetryReport, PROTOCOL_VERSION};
...
`ReconReport` is what `gardyn-edge probe` writes, and it is the only record of what the
...
```rust
use gardyn_proto::topics;

assert_eq!(
    topics::for_garden(topics::TELEMETRY, "abc-123"),
    "gardyn/abc-123/telemetry"
);
```
