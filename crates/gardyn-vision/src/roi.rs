...
//! area of slot 7 without knowing which pixels are slot 7. `gardyn-cli vision
//! calibrate` is what produces one.

use gardyn_core::{Geometry, LensCalibration, SlotId};
...
impl RoiMap {
    ...
    /// `gardyn-cli vision calibrate` has something to draw on the first frame.
    ///
