...
-- for the plants.
--
-- The payload is the JSON of `garden_core::TankEvent`, so adding a kind of event does
...
-- one use. See garden_auth::action.
...
-- Where each slot is in the camera frame, as a `garden_vision::RoiMap` document.
--
...
-- two settings that can disagree. `garden-cli vision calibrate` writes it.
