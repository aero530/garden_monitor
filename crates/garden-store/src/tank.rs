...
//! `garden-cli replay` can rebuild what the tank looked like on any past day, which is
//! what makes replaying history against a modified rule honest.

use crate::{Result, Store, StoreError, ts};
use garden_auth::UserId;
use garden_core::{GardenId, TankEvent, TankGeometry, TankState};
...
mod schedule_tests {
    ...
    async fn one_gardens_schedule_is_invisible_to_another() {
        ...
        let theirs = store
            .create_garden(
                "Theirs",
                garden_core::DeviceModel::Studio2,
                "UTC",
                user,
                t0(),
            )
