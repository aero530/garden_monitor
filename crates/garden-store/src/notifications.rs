...
use garden_auth::{SecretToken, UserId};
use garden_core::{GardenId, Severity, TaskKey};
use garden_notify::{LastNotified, QuietHours};
use jiff::Timestamp;
use sqlx::Row;
use uuid::Uuid;
...
mod tests {
    use super::*;
    use garden_core::{Target, TaskKind};
    ...
    async fn preferences_round_trip() {
        ...
        prefs.ntfy_topic = Some("garden-phil-a8f3".into());
        ...
        assert_eq!(back.ntfy_topic.as_deref(), Some("garden-phil-a8f3"));
    ...
    async fn issuing_a_feed_does_not_clobber_existing_preferences() {
        ...
        let (store, _, user) = fixture().await;
        let mut prefs = NotificationPrefs::default_for(user);
        prefs.ntfy_topic = Some("garden-phil".into());
        store.save_notification_prefs(&prefs).await.unwrap();

        store.issue_calendar_feed(user).await.unwrap();

        let back = store.notification_prefs(user).await.unwrap();
        assert_eq!(back.ntfy_topic.as_deref(), Some("garden-phil"));
    ...
    async fn the_most_recent_send_is_the_one_that_counts() {
        ...
        store
            ...
            .await
            .unwrap();
        let later = garden_core::time::add_days(t0(), 1.0);
        store
    ...
    async fn one_persons_history_is_not_anothers() {
        ...
        let sam = store
            .create_user(
                garden_auth::EmailAddress::parse("sam@example.com").unwrap(),
                "Sam",
                "a long enough password",
                t0(),
            )
