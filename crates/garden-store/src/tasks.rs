...
use garden_auth::{ActionGrant, ActionGrantId, GrantError, SecretToken, TaskAction, UserId};
use garden_core::{GardenId, Severity, Task, TaskKey};
use jiff::Timestamp;
use sqlx::Row;
...
impl Store {
                ...
                Some(row) => {
                    ...
                    let unverified = state == TaskState::Done
                        && completed_at.is_some_and(|c| {
                            garden_core::time::days_between(c, now)
    ...
    pub async fn redeem_action_grant(
        ...
        let mut grant = ActionGrant {
            ...
            digest: garden_auth::TokenDigest::from_stored(row.try_get::<String, _>("digest")?),
