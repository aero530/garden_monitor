...
fn harvest_task(now: jiff::Timestamp) -> Task {
    Task::new(
        TaskKind::Harvest,
        Target::Slot(garden_core::SlotId(3)),
        Severity::Advisory,
...
async fn garden() -> (Store, GardenId, garden_auth::UserId) {
...
async fn a_backup_is_a_readable_copy_taken_from_a_live_database() {
    ...
    let source_path = dir.join(format!("garden-src-{unique}.db"));
    let backup_path = dir.join(format!("garden-backup-{unique}.db"));
