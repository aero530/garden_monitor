...
use garden_store::frames::{FrameError, FrameSource, ImageKind, MAX_FRAME_BYTES, NewFrame};
...
fn frame_at<'a>(garden: GardenId, bytes: &'a [u8], minutes: f64) -> NewFrame<'a> {
    NewFrame {
        garden,
        captured_at: garden_core::time::add_days(t0(), minutes / (24.0 * 60.0)),
        width: 640,
        height: 480,
...
async fn pruning_removes_old_frames_and_their_bytes() {
    ...
    let old = store
        ...
        .unwrap()
        .unwrap();
    let fresh_at = garden_core::time::add_days(t0(), 20.0);
    let fresh = store
        .put_frame(NewFrame {
    ...
    let now = garden_core::time::add_days(t0(), 21.0);
