...
```sh
cargo test -p garden-vision                      # 68 tests
cargo test -p garden-vision --features diagnosis # + the Ollama backend
```
...
```rust
garden_vision::apply_growth(&mut report, &history, now);
```

Calibration is [`garden-cli vision`](../garden-cli/); running it against real uploads is
[`garden-web`](../garden-web/).
