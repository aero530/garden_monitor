...
//! 3. **Two writers are arbitrated by a marker file.** `gardyn-guard` seizes the pins
...
use gardyn_hal::{Duty, GuardMarker, Setpoint};
...
use gardyn_proto::recon::expected::{GPIO_LIGHT, GPIO_PUMP};
...
mod tests {
    ...
    fn marker(name: &str) -> GuardMarker {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        GuardMarker::new(std::env::temp_dir().join(format!("gardyn-guard-{name}-{nanos}")))
    }
