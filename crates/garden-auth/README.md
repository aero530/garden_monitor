...
```sh
cargo test -p garden-auth     # 78 tests
```
...
```rust
use garden_auth::{Actor, Permission};
...
```rust
use garden_auth::{ActionGrant, TaskAction};
...
```rust
use garden_auth::accept_new_password;
...
[`garden-store/tests/tenancy.rs`](../garden-store/tests/tenancy.rs); this crate proves
