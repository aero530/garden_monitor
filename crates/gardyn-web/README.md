...
flowchart TD
  subgraph in["inbound"]
    ...
    pi["gardyn-edge<br/><small>bearer token</small>"]
    phone["notification button<br/><small>signed link, no login</small>"]
  end

  subgraph app["gardyn-web"]
    direction TB
  ...
  store[("<b>gardyn-store</b>")]
  rules["<b>gardyn-rules</b>"]
  notify["<b>gardyn-notify</b>"]
...
4. For each task, `gardyn-notify::decide`, then deliver.
...
Bearer token, `GARDYN_AGENT_TOKEN`. Closed when unset, and it logs that at startup.
...
| `GARDYN_DATA_DIR` | `gardyn-data` | frames land in `$GARDYN_DATA_DIR/frames` |
...
| `GARDYN_INSECURE_COOKIES` | *unset* | development only |
| `GARDYN_NTFY_URL` / `_TOKEN` | *unset* | no push when unset |
| `GARDYN_SMTP_*` | *unset* | no email when unset |
| `RUST_LOG` | `gardyn_web=info,tower_http=warn` | |

`GARDYN_BASE_URL` is the one people get wrong. Invite links and notification buttons are
