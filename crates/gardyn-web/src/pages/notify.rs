...
use gardyn_auth::{Permission, SecretToken};
use gardyn_notify::{CalendarTask, QuietHours, render_calendar};
            ...
            @if !configured {
                div.card {
                    ...
                    p.muted.small style="margin:0" {
                        "The administrator has not set " code { "GARDYN_NTFY_URL" }
...
async fn calendar_feed(
    ...
    for listing in &listings {
        ...
        for task in state.store.tasks_for(listing.garden.id).await? {
            ...
            entries.push(CalendarTask {
                ...
                // every refresh.
                uid: format!("{}-{}@gardyn", task.key, listing.garden.id),
    ...
    Ok((
        [
            (CONTENT_TYPE, gardyn_notify::calendar::CONTENT_TYPE),
