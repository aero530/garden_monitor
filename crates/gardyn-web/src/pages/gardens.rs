...
use gardyn_auth::{Actor, Permission, Role};
use gardyn_core::{DeviceModel, Garden, GardenId, GardenState};
use gardyn_store::gardens::GardenListing;
...
pub async fn authorize(
    ...
    let garden = state
        ...
        .ok_or(AppError::Denied(gardyn_auth::AccessDenied::NotAMember {
            garden: id,
        }))?;
    Ok((garden, role))
}
...
async fn create(
    ...
    let created_at = if model == DeviceModel::Simulated {
        gardyn_core::time::add_days(now, -SIMULATED_GARDEN_AGE_DAYS)
    } else {
        now
    };
