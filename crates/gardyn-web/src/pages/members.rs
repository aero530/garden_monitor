...
use gardyn_auth::{Actor, EmailAddress, Invitation, Membership, Permission, Role, UserId};
use gardyn_core::{Garden, GardenId};
...
async fn invite(
    ...
    if !role.can_grant(target) {
        return Err(AppError::Denied(gardyn_auth::AccessDenied::InsufficientRole {
            garden: id,
            held: role,
            required: Role::Owner,
