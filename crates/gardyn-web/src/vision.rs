...
use gardyn_core::{GardenId, SlotId, Timestamp};
use gardyn_store::Store;
use gardyn_vision::{Analyzer, growth, roi::RoiMap};
...
pub async fn analyse_and_store(
    ...
    gardyn_vision::apply_growth(&mut report, &history, at);
...
async fn load_history(
    store: &Store,
    garden: GardenId,
    slots: &[gardyn_core::SlotMetrics],
    now: Timestamp,
) -> BTreeMap<SlotId, Vec<growth::Sample>> {
    let since = gardyn_core::time::add_days(now, -growth::WINDOW_DAYS);
...
mod tests {
    ...
    use gardyn_store::frames::{FrameSource, NewFrame};
