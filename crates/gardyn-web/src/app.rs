...
use gardyn_auth::{Actor, SecretToken, session};
...
mod tests {
    ...
    fn a_single_cookie_is_read() {
        let headers = headers_with("gardyn_session=abc123");
        assert_eq!(read_cookie(&headers, "gardyn_session"), Some("abc123"));
    }

    #[test]
    fn the_right_cookie_is_picked_out_of_several() {
        let headers = headers_with("theme=dark; gardyn_session=abc123; other=x");
        assert_eq!(read_cookie(&headers, "gardyn_session"), Some("abc123"));
    ...
    fn a_missing_cookie_header_is_not_an_error() {
        assert_eq!(read_cookie(&HeaderMap::new(), "gardyn_session"), None);
    }

    #[test]
    fn session_cookies_are_locked_down() {
        let cookie = set_cookie("gardyn_session", "abc", 3600, true);
    ...
    fn plain_http_omits_secure_so_local_development_works() {
        let cookie = set_cookie("gardyn_session", "abc", 3600, false);
    ...
    fn clearing_a_cookie_expires_it_immediately() {
        assert!(clear_cookie("gardyn_session", true).contains("Max-Age=0"));
    }
