...
```sh
cargo test -p garden-notify   # 44 tests
```
...
```rust
use garden_core::Severity;
use garden_notify::reach_for;
...
```rust
use garden_notify::{Decision, HoldReason, decide};
...
- **At most three interrupting notifications per garden per sweep**, most severe first.
  ...
  itself lives in the dispatcher, in `garden-web`.)

## Composing and sending

```rust
use garden_notify::{Notifier, compose, reach_for};
...
The buttons are single-use signed links from `garden-auth::ActionGrant`, needing no
login. They are also the reason `GARDEN_BASE_URL` has to be a name your **phone** can
resolve: get it wrong and push arrives perfectly while every button is dead.

## Calendar feed

```rust
use garden_notify::{CalendarTask, render_calendar, calendar::CONTENT_TYPE};
