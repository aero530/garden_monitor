...
```sh
cargo run -p gardyn-sim          # 120-day season
cargo run -p gardyn-sim 365      # a year
cargo test -p gardyn-sim         # 32 tests
```
...
flowchart LR
  ...
  engine["<b>gardyn-rules</b><br/>Engine::evaluate"]
...
use gardyn_sim::{Simulation, scenario::{Operator, run}};
...
`cargo run -p gardyn-sim` prints the capability report, an operator comparison, and the
hardware comparison:
