...
//! `CARGO_BIN_EXE_gardyn-cli`.

use gardyn_auth::EmailAddress;
use gardyn_core::{DeviceModel, GardenId, SlotId, Timestamp, VarietyId, time::add_days};
...
async fn fixture(name: &str) -> Fixture {
    let dir = std::env::temp_dir().join(format!("gardyn-cli-{name}-{}", uuid::Uuid::new_v4()));
...
fn run(fixture: &Fixture, args: &[&str]) -> String {
    let output = Command::new(env!("CARGO_BIN_EXE_gardyn-cli"))
        .args(args)
        .env("GARDYN_DB", &fixture.url)
        .output()
        .expect("run gardyn-cli");
...
async fn a_bad_garden_id_explains_how_to_find_a_good_one() {
    ...
    assert!(err.contains("gardyn-cli gardens"), "{err}");
}
...
fn tank_calibration_needs_no_database_at_all() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_gardyn-cli"))
        ...
        .env("GARDYN_DB", "sqlite:///nonexistent/path/nope.db")
        .output()
        .expect("run gardyn-cli");
