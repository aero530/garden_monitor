...
pub async fn run(
    ...
    to: Timestamp,
    extra: &[Capability],
) -> gardyn_store::Result<Summary> {
    let mut summary = Summary::default();
    let mut previous: Vec<String> = Vec::new();

    let span = gardyn_core::time::days_between(from, to).max(0.0);
...
mod tests {
    ...
    use gardyn_core::{DeviceModel, SlotId, TankEvent, VarietyId};
    ...
    async fn a_planting_produces_a_harvest_task_on_the_day_it_matures() {
        ...
        let day = gardyn_core::time::days_between(t0(), *first);
    ...
    async fn a_pulled_plant_stops_producing_tasks_from_the_day_it_was_pulled() {
        ...
        plant_kale(&store, garden, t0()).await;
        store
            .remove_planting(garden, gardyn_core::PlantingId(1), add_days(t0(), 20.0))
            .await
            .unwrap();
