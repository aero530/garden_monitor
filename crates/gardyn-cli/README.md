...
```sh
cargo run -p gardyn-cli -- gardens
cargo test -p gardyn-cli    # 37 tests, 14 of them running the built binary
```
...
```sh
gardyn-cli vision init --garden $ID --width 1920 --height 1080
gardyn-cli vision preview --frame frame.jpg      # writes rois-preview.png
#   ...edit rois.json, repeat until the boxes sit over the yPods...
gardyn-cli vision scale --cm 7 --px 70           # a yPod you measured with a ruler
gardyn-cli vision test --frame frame.jpg         # what it would measure
gardyn-cli vision apply --garden $ID             # switches vision on
```
...
```sh
gardyn-cli log feed --garden $ID
gardyn-cli log top-off --garden $ID --litres 2
gardyn-cli log clean --garden $ID --days-ago 3
gardyn-cli log show --garden $ID
gardyn-cli log undo --garden $ID <event-id>
```
...
```sh
gardyn-cli schedule set --garden $ID --hours 14 --duty 0.85 --ramp 30
gardyn-cli schedule preview --garden $ID     # the whole day, hour by hour
```
...
```sh
gardyn-cli replay --garden $ID --days 90
gardyn-cli replay --garden $ID --days 90 --capability conductivity --verbose
```
...
`--database` defaults to `sqlite://gardyn.db` and reads `GARDYN_DB`, so it picks up the
