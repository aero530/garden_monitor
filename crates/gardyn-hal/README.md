...
```sh
cargo test -p gardyn-hal      # 24 tests
```
...
- **The dawn and dusk ramps** are what `gardyn-edge watch-pwm` exists to record. A step
...
`gardyn-edge` drives the garden; `gardyn-guard` takes over when it stops beating. They
...
```rust
use gardyn_hal::{GuardMarker, Heartbeat};
