...
//!
//! `gardyn-edge` drives the garden from its resident schedule. `gardyn-guard` takes
...
impl GuardMarker {
    pub const DEFAULT_PATH: &'static str = "/run/gardyn/guard.engaged";
    ...
    pub fn engage(&self) -> std::io::Result<()> {
        ...
        std::fs::write(&self.path, b"gardyn-guard\n")
    }
...
impl Heartbeat {
    pub const DEFAULT_PATH: &'static str = "/run/gardyn/edge.heartbeat";
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        std::env::temp_dir().join(format!("gardyn-handover-{name}-{nanos}"))
    }
    ...
    fn touching_creates_the_run_directory() {
        // /run/gardyn does not exist on a fresh boot, and the agent must not need a
