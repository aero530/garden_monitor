...
//!
//! The unit tests in `gardyn-auth` prove the policy is right. These prove the queries
...
use gardyn_auth::{Actor, EmailAddress, Invitation, Membership, Permission, Role};
use gardyn_core::DeviceModel;
...
async fn actor_for(store: &Store, user: gardyn_auth::UserId) -> Actor {
...
async fn an_address_can_only_register_once() {
    ...
    let again = store
        ...
        .await;
    assert!(matches!(again, Err(gardyn_store::StoreError::EmailTaken)));
}
...
async fn a_garden_cannot_be_handed_to_a_stranger() {
    ...
    let result = store
        ...
        .await;
    assert!(matches!(result, Err(gardyn_store::StoreError::NotFound)));
    assert_eq!(
        store.role_of(garden.id, phil.id).await.unwrap(),
        Some(Role::Owner),
...
async fn an_expired_session_stops_working() {
    ...
    let much_later = gardyn_core::time::add_days(t0(), 60.0);
