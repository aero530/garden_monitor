...
`garden-edge` touches a heartbeat file. If it stops for long enough, guard seizes the
PWM lines and runs a conservative schedule until someone notices.

```sh
cargo test -p garden-guard    # 10 tests
```
...
> before you trust a watchdog look like. Clearing `GARDEN_GUARD_DRY_RUN` is what makes
> it real.
...
```mermaid
flowchart LR
  edge["<b>garden-edge</b>"] -->|"touch, every tick"| hb[("/run/garden/edge.heartbeat")]

  subgraph guard["garden-guard — every 15 s"]
    direction TB
...
out the simple one.** `garden-edge` does I²C, HTTP, JSON, image capture and disk
...
| 2 | `garden-failsafe.service` boot defaults | the agent never starting |
| **3** | **`garden-guard`** | **the agent starting, then dying or hanging** |
...
The duty goes through `garden_hal::Duty::pump`, so even a wrong constant here cannot
exceed the 30% supply ceiling.

## The handover

The guard writes `/run/garden/guard.engaged` when it seizes the pins, and the agent
...
```sh
garden-guard \
  --heartbeat /run/garden/edge.heartbeat \
  --grace-seconds 300 \
  --interval-seconds 15
```
...
| Flag | Env | Default | |
|---|---|---|---|
| `--heartbeat` | `GARDEN_HEARTBEAT` | `/run/garden/edge.heartbeat` | file the agent touches |
| `--grace-seconds` | `GARDEN_GRACE_SECONDS` | `300` | silence before the agent is presumed dead |
| `--interval-seconds` | — | `15` | how often to look |
| `--dry-run` | `GARDEN_GUARD_DRY_RUN` | **`true`** | log only; the default until Phase 6 |
| `--marker` | `GARDEN_GUARD_MARKER` | `/run/garden/guard.engaged` | how the agent is told to stand down |
...
- [ ] Guard has run in dry-run for weeks and logged sensible setpoints across a full
      light cycle
- [ ] You have deliberately killed `garden-edge` and confirmed guard notices
